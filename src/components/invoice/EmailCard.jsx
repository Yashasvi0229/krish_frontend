...
function AttachmentRow({ att, onOpen }) {
  ...
  // Attachment download uses an authenticated fetch → blob → local URL
  // (rather than a plain anchor) because the backend requires a JWT.
  const handleDownload = async (e) => {
    e.stopPropagation();
    try {
      const token = localStorage.getItem('gnc_access_token');
      const url = attachmentApi.downloadUrl(att.attachment_id || att.id);
  ...
  return (
    <div className="w-full border border-slate-200 rounded-md p-3 hover:border-primary transition-colors">
      ...
      {att.summary && (
        <button
          onClick={onOpen}
          className="w-full text-left text-small text-slate-600 italic mt-2 line-clamp-2"
        >
          {att.summary}
        </button>
      )}
