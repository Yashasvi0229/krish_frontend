 ...
 *
 * WHY baseURL is just '/api':
 *   Vite dev proxy (vite.config.js) forwards /api → http://localhost:8000.
 *   In production the frontend is served from the same origin as the API,
 *   so the relative URL always works — no env variable juggling needed.
 *
 * WHY interceptors:
 *   1. Attach JWT from localStorage on every request.
 *   2. Auto-redirect on 401 (session expired). We deliberately don't clear
 *      auth state ourselves on other 4xx; components handle those with
 *      their own toasts to keep messages contextual.
 */
